                ...
                @foreach ($imgList as $img)
                    <div class="col-md-4">
                        <div class="card mb-4 h-100">
                            <img src="{{ asset('storage/' . $img->image) }}" class="card-img-top" alt=""
                                style="height: 250px">
                            <div class="card-body">
                                <h5 class="card-title text-center">{{ $img->title }}</h5>
                                <div class="text-center">
                                    <a href="{{ route('admin#editImage', $img->id) }}"><button
                                            class="btn bg-light shadow-sm" title="edit"><i
                                                class="fa-solid fa-pen-to-square text-dark"></i></button></a>
                                    <a href="{{ route('admin#deleteImage', $img->id) }}"><button
                                            class="btn bg-light shadow-sm" title="delete"><i
                                                class="fa-solid fa-trash text-dark"></i></button></a>
                                </div>
                            </div>
                        </div>
                    </div>
